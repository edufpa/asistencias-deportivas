...
export default function AsistenciaPage() {
  ...
  const loadSession = useCallback(async (sid: string) => {
    ...
    // Get session details + records + convocatoria players
    ...
    const recordMap = new Map(
      existingRecords.map((r: { playerId: string; status: string; performanceScore: number | null; absenceReason: string | null }) => [
        r.playerId,
        r,
      ])
    );
  ...
  async function handleStartSession() {
    ...
    const recRes = await fetch(
      `/api/convocatorias/${convocatoriaId}/sessions/${sessionData.id}/records`
    );
    const existingRecords = await recRes.json();
    const recordMap = new Map(
  ...
  async function handleSave() {
    ...
    const res = await fetch(
      `/api/convocatorias/${convocatoriaId}/sessions/${sessionId}/records`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  if (loadingSession) {
    return <div className="text-gray-400">Cargando sesión...</div>;
  }
      ...
      {step === "attendance" && (
        <>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-500 text-sm">
                {format(new Date(sessionDate + "T12:00:00"), "EEEE d 'de' MMMM yyyy", {
                  locale: es,
                })}
              </span>
            </div>
          ...
          <div className="text-xs text-gray-400 bg-gray-50 rounded-lg px-4 py-2 border">
            <strong>Instrucciones:</strong> Seleccioná el estado para cada jugador. Si dejás en
            blanco, significa que el jugador no debía asistir a esta sesión.
          </div>
                ...
                <CardContent className="py-4">
                  <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                    <div className="sm:w-48 shrink-0">
                      ...
                      {record.club && (
                        <p className="text-xs text-gray-400 mt-0.5">{record.club}</p>
                      )}
                    </div>
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          type="button"
                          onClick={() => updateRecord(record.playerId, "status", null)}
                        ...
                        <button
                          type="button"
                          onClick={() => updateRecord(record.playerId, "status", "ATTENDED")}
                          className={`px-3 py-1.5 rounded-md border text-sm font-medium transition-colors ${
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            updateRecord(record.playerId, "status", "ABSENT_JUSTIFIED")
                          }
                          className={`px-3 py-1.5 rounded-md border text-sm font-medium transition-colors ${
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            updateRecord(record.playerId, "status", "ABSENT_UNJUSTIFIED")
                          }
                          className={`px-3 py-1.5 rounded-md border text-sm font-medium transition-colors ${
                      ...
                      {/* Performance score for ATTENDED */}
                      {record.status === "ATTENDED" && (
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex gap-1">
                            {[1, 2, 3, 4].map((score) => (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  updateRecord(record.playerId, "performanceScore", score)
                                }
                          ...
                          {record.performanceScore && (
                            <span className="text-xs text-gray-500">
                              {SCORE_LABELS[record.performanceScore]}
                            </span>
                          )}
                        </div>
                      )}

                      {/* Absence reason for JUSTIFIED */}
                      {record.status === "ABSENT_JUSTIFIED" && (
                        <div className="space-y-1">
                          ...
                          <Textarea
                            ...
                            onChange={(e) =>
                              updateRecord(record.playerId, "absenceReason", e.target.value)
                            }
          ...
          <div className="flex justify-between items-center pt-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/convocatorias/${convocatoriaId}`)}
            >
